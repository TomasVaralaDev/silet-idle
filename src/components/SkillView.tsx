...
export default function SkillView({ skill }: SkillViewProps) {
  ...
  if (!definition)
    return <div className="p-10 text-red-500">Config missing</div>;
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between mb-4 w-full relative">
                  <div
                    className={`min-w-[3.5rem] min-h-[3.5rem] rounded-lg bg-slate-950 flex flex-wrap items-center justify-center border border-slate-800 shrink-0 relative overflow-hidden gap-1 p-1`}
                  >
                    {hasDrops ? (
                  ...
                  {!isUnlocked ? (
                    ...
                    <span className="px-2 py-1 rounded bg-orange-900/20 text-orange-400 text-[10px] font-bold border border-orange-900/30 ml-2">
                      COST
                    </span>
                  ) : null}
                </div>
                ...
                <div className="mb-4 flex-1">
                  <h3
                    className={`font-bold text-sm ${!isDisabled ? 'text-slate-200' : 'text-slate-600'}`}
                  >
                    {resource.name}
                  </h3>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {resource.inputs.map((input) => {
                        ...
                        return (
                          <div
                            key={input.id}
                            className={`text-xs flex items-center gap-1 ${hasEnough ? 'text-slate-300' : 'text-red-400'}`}
                          >
                            <img
                              src={item?.icon}
                ...
                {isUnlocked && (
                  <div className="mt-auto pt-3 border-t border-slate-800/50 w-full">
                    <div className="flex justify-between items-center text-xs mb-2">
                      <span
                        className={`font-bold ${globalXpMultiplier > 1 ? 'text-emerald-400' : 'text-cyan-400'}`}
                      >
                        +{effectiveXp.toFixed(1)} XP
                      </span>
                      <span
                        className={`font-bold ${globalSpeedMultiplier > 1 ? 'text-emerald-400' : 'text-slate-500'}`}
                      >

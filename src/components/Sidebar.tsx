...
import type { ViewType } from '../types'; // SkillType poistettu tästä
...
const NavButton = ({ view, label, iconPath, level, currentView, onClick }: NavButtonProps) => {
  ...
  if (isActive) {
     ...
     else if (view === 'scavenger') baseStyle = 'bg-indigo-900/40 text-indigo-400 border-indigo-500/40 shadow-[0_0_15px_rgba(99,102,241,0.15)] font-bold'; 
...
export default function Sidebar({ currentView, setView, onReset, onLogout, onStopAction, onForceSave, onOpenSettings }: SidebarProps) {
          ...
          <div className="relative" ref={menuRef}>
            <button onClick={() => setIsProfileOpen(!isProfileOpen)} className="flex items-center gap-3 hover:bg-slate-900 p-2 rounded border border-slate-800 transition-all">
              <div className="text-right hidden sm:block">
                {/* KORJAUS: username otettu käyttöön tässä */}
              ...
              <div className="w-10 h-10 bg-slate-900 rounded border border-slate-700 flex items-center justify-center relative overflow-hidden">
                  <img src="/assets/ui/icon_user_avatar.png" alt="User" className="pixelated" 
                       onError={(e) => e.currentTarget.src = 'https://ui-avatars.com/api/?name=U&background=0f172a'} />
              </div>
            </button>
            {isProfileOpen && (
              <div className="absolute top-full right-0 mt-2 w-64 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl z-[100] p-2">
                <button onClick={() => { setIsProfileOpen(false); onOpenSettings(); }} className="w-full text-left px-3 py-2 rounded hover:bg-slate-800 text-xs font-bold text-slate-300 flex items-center gap-3">System Config</button>
                <button onClick={onLogout} className="w-full text-left px-3 py-2 rounded hover:bg-red-900/20 text-xs font-bold text-red-400 flex items-center gap-3">Terminate Session</button>
              </div>
            )}
          </div>
        </div>

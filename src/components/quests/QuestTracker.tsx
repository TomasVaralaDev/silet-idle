            ...
            return (
              <div key={quest.id} className="group">
                <div className="flex justify-between text-[11px] mb-1">
                  <span className={`font-bold truncate pr-2 ${isDone ? 'text-emerald-400' : 'text-slate-300'}`}>
                    {quest.title}
                  </span>
                ...
                {isDone && (
                  <p className="text-[9px] text-emerald-500 font-bold mt-1 animate-pulse uppercase">Ready to claim!</p>
                )}
              </div>
            );
          })}
